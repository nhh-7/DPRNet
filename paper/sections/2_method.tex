...
\end{equation}
where $s\in\{2,3,4\}$ is the scale factor. The proposed change is confined to TAB:
we keep its external interface
$\mathbb{R}^{C\times H\times W}\!\to\!\mathbb{R}^{C\times H\times W}$ unchanged and
replace the original means-based center iteration and hard-label sorting with
the DPR pipeline (Section~\ref{sec:method_dpr}), while reusing the subsequent
intra/inter-group aggregation operator (IASA, Section~\ref{sec:method_iasa}). This
...
design leaves four weaknesses that DPR targets. (i) The centers are driven by a
...
label and discards assignment confidence. (iii) The ordering clusters by label
only, with no intra-cluster purity constraint. (iv) Prototype generation and
prototype confirmation are entangled, which weakens interpretability and makes the
routing state harder to analyze. DPR addresses these issues by generating prototypes
from the current tokens, decoupling generation from confirmation, and propagating an
explicit assignment confidence through routing and aggregation.
...
sequence, $M$ dynamic prototypes, and routing metadata. The pipeline contains four
stages (Figure~\ref{fig:dpr}).
...
\begin{equation}
A = \mathrm{softmax}\big(E W_a\big),
\label{eq:assign}
\end{equation}
where $W_a\in\mathbb{R}^{C\times M}$, the soft assignment
$A\in\mathbb{R}^{B\times N\times M}$, and the softmax is taken over the $M$ slots. Each prototype is the
...
To separate prototype \emph{generation} from \emph{confirmation}, we refine
$P^{\mathrm{c}}$ with a learnable query bank
...
the start of training, which helps keep prototype identities stable across samples.
...
\label{eq:scores}
\end{equation}
where $S\in\mathbb{R}^{B\times N\times M}$ and $\theta$ is a learnable log-scale
clamped by $\tau_{\max}$ for stability.
...
the hard-label confidence discarded by TAB).
...
\qquad
\pi = \operatorname{argsort}_n(k_n).
\label{eq:sortkey}
\end{equation}
...
are ordered by confidence. The token sequence, labels, and confidences are
...
local attention over overlapping groups of size $g$, which captures fine local
...
To stabilize tokens whose hard assignment is uncertain, DPR adds a soft,
confidence-weighted prototype context to the aggregated output. With a soft summary
...
through the $1\times1$ convolution, residual connection, and ConvFFN.
...
A learnable temperature can concentrate routing on a few prototypes. We therefore
add a usage-entropy regularizer that encourages balanced slot use. Let
$u_m=\frac{1}{N}\sum_n S_{n,m}$ be the mean usage of prototype $m$;
...
\label{eq:balance}
\end{equation}
which is largest, and therefore gives the smallest auxiliary loss, when usage is
uniform. The total objective adds this term, summed over all blocks, to the pixel
reconstruction loss:
\begin{equation}
...
DPR keeps the backbone interface and parameter budget in the lightweight regime
while changing only how tokens are organized before aggregation. The four stages map
directly to the contributions: dynamic generation (Eqs.~\eqref{eq:assign}--%
\eqref{eq:proto_content}) for C1, generation/confirmation decoupling
(Eq.~\eqref{eq:proto_refine}) for C2, confidence-aware routing
(Eqs.~\eqref{eq:scores}--\eqref{eq:sortkey}, \eqref{eq:iasa_gate}--\eqref{eq:soft_fallback})
for C3, and the learnable temperature with usage balancing (Eqs.~\eqref{eq:scores},
\eqref{eq:balance}) for C4. Because each design element is exposed as an independent
switch that defaults to the full model and has a defined disabled form, the

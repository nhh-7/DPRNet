...
$e^{6}$ in log-scale form and clamped at $\tau_{\max}{=}10$
...
finetune the $\times3$ and $\times4$ models for $250$k iterations each from the
$\times2$ checkpoint. Training patches are $128{\times}128$, $192{\times}192$,
...
best benchmark-averaged PSNR/SSIM over Set5--Manga109, without per-dataset
checkpoint selection. Experiments use PyTorch with the BasicSR
...
To compare with FLOPs-based reports, the Multi-Adds values are multiplied by two.

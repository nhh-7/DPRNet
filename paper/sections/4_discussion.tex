...
the ablations (Sec.~\ref{sec:ablation}) initialize every variant from the same converged
full-model state under a short $80$k-iteration, single-seed budget; this isolates
...
attribution would require retraining each variant from a switch-neutral initialization
under the CATANet $\times4$ full-budget ablation protocol. Second, the motivation for dynamic, per-image

...
keeping it within the lightweight bracket alongside the compared methods---in
particular below the lightweight Transformer baselines SwinIR-light ($60.3$\,G) and
SRFormer-light ($56.5$\,G) under the same input convention---while delivering top-tier
...
bank and router projections of DPR, so we make no efficiency claim against CATANet; the
per-scale profile is given in Table~\ref{tab:efficiency_profile}. A complete
Multi-Adds/latency comparison against all competitors under our measurement protocol is
left for the final version, as the remaining numbers are not yet collected under a
unified setting.

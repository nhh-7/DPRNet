...
To inspect what DPR actually does at routing time---rather than its effect on
PSNR---we analyze the routing diagnostics logged during validation at $\times4$
...
prototypes increases from $47.5$ to $52.3$ across blocks, i.e.\ fewer slots are
...
and prevents slot collapse, which is exactly the stabilizing role claimed for C4.
...
fallback (Eq.~\eqref{eq:soft_fallback}) is logged as a non-degenerate signal in
every block (block-mean $x^{\mathrm{s}}$ on the order of $10^{-2}$, well above the
uniform-assignment floor $1/M$ for the large-$M$ blocks), confirming that the
softmax assignment is informative rather than flat and that the confidence used by
the C3 components is meaningful. A finer-grained, per-token view of this signal
(the $x^{\mathrm{s}}$ histogram before vs.\ after the learnable temperature, and
the routing cluster maps obtained by reshaping the original-order labels $b_n$ to
$H\times W$) requires a forward pass over the test images; the model already caches
the original-order routing map and confidences for this purpose
(\texttt{last\_routing\_map}, \texttt{last\_x\_scores}), and we present these
qualitative visualizations together with the visual comparison in the camera-ready,
keeping the quantitative interpretability claims in this section grounded in the
logged statistics.

\paragraph{Summary.}
The logged diagnostics give direct, mechanism-level evidence for the routing
design: the balancing regularizer produces a block-consistent increase in usage
